 ...
 * </p>
 */
public class PagerTitleStripNew extends ViewGroup implements ViewPager.Decor {
	ViewPager mPager;
	TextView mPrevText;
	TextView mCurrText;
	TextView mNextText;
	...
	public PagerTitleStripNew(Context context) {
		this(context, null);
	}

	public PagerTitleStripNew(Context context, AttributeSet attrs) {
		super(context, attrs);
		...
		mPrevText.setTypeface(type);
		mCurrText.setTypeface(type,1);
		mNextText.setTypeface(type);
	...
	protected void onAttachedToWindow() {
		...
		mPager = pager;
		updateAdapter(mWatchingAdapter != null ? mWatchingAdapter.get() : null,
				adapter);
	}
	...
	void updateText(int currentItem, PagerAdapter adapter) {
		...
		mCurrText.setText(adapter != null && currentItem < itemCount ? adapter
				.getPageTitle(currentItem) : null);
	...
	void updateTextPositions(int position, float positionOffset, boolean force) {
		...
		final int maxBaseline = Math.max(Math.max(prevBaseline, currBaseline),
				nextBaseline);
		...
		final int alignedPrevHeight = prevTopOffset
				+ mPrevText.getMeasuredHeight();
		final int alignedCurrHeight = currTopOffset
				+ mCurrText.getMeasuredHeight();
		final int alignedNextHeight = nextTopOffset
				+ mNextText.getMeasuredHeight();
		final int maxTextHeight = Math.max(
				Math.max(alignedPrevHeight, alignedCurrHeight),
				alignedNextHeight);
		...
		case Gravity.CENTER_VERTICAL:
			...
			break;
		case Gravity.BOTTOM:
			final int bottomGravTop = stripHeight - paddingBottom
			- maxTextHeight;
		...
		mPrevText.layout(prevLeft, prevTop, prevLeft + prevWidth, prevTop
				+ mPrevText.getMeasuredHeight());

		final int nextLeft = Math.max(stripWidth - paddingRight - nextWidth,
				currRight + mScaledTextSpacing);
		mNextText.layout(nextLeft, nextTop, nextLeft + nextWidth, nextTop
				+ mNextText.getMeasuredHeight());
